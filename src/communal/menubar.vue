<template>
  <a-layout>
    <a-layout-header class="header">
      <div class="header-right">
        <label class="switch">
          <input v-model="isDarkMode" @change="toggleTheme" id="checkbox" type="checkbox" />
            <span class="slider">
              <div class="star star_1"></div>
              <div class="star star_2"></div>
              <div class="star star_3"></div>
              <svg viewBox="0 0 16 16" class="cloud_1 cloud">
                <path transform="matrix(.77976 0 0 .78395-299.99-418.63)" fill="#fff" d="m391.84 540.91c-.421-.329-.949-.524-1.523-.524-1.351 0-2.451 1.084-2.485 2.435-1.395.526-2.388 1.88-2.388 3.466 0 1.874 1.385 3.423 3.182 3.667v.034h12.73v-.006c1.775-.104 3.182-1.584 3.182-3.395 0-1.747-1.309-3.186-2.994-3.379.007-.106.011-.214.011-.322 0-2.707-2.271-4.901-5.072-4.901-2.073 0-3.856 1.202-4.643 2.925">
                </path>
              </svg>
            </span>
          </label>
        <a-dropdown>
          <div class="user-info">
            <a-avatar src="https://www.antdv.com/assets/logo.1ef800a8.svg" />
            <span class="username">{{ username }}</span>
          </div>
          <template #overlay>
            <a-menu>
              <a-menu-item key="1">
                <user-outlined />个人中心
              </a-menu-item>
              <a-menu-item key="2" @click="handleLogout">
                <logout-outlined />退出登录
              </a-menu-item>
            </a-menu>
          </template>
        </a-dropdown>
      </div>
    </a-layout-header>
    <a-layout-sider width="200" style="background: #fff">
      <a-menu mode="inline" theme="dark" v-model:selectedKeys="selectedKeys" v-model:openKeys="openKeys" style="height: 100vh">
        <a-menu-item key="home">
          <template #icon>
            <home-outlined />
          </template>
          <router-link to="/">数据</router-link>
        </a-menu-item>
        <a-sub-menu key="management">
          <template #icon>
            <setting-outlined />
          </template>
          <template #title>列表管理</template>
          <a-menu-item key="users">
            <template #icon>
              <team-outlined />
            </template>
            <router-link to="/management/users">用户管理</router-link>
          </a-menu-item>
          <a-menu-item key="roles">
            <template #icon>
              <safety-outlined />
            </template>
            <router-link to="/management/roles">角色管理</router-link>
          </a-menu-item>
        </a-sub-menu>
        <a-menu-item key="logs">
          <template #icon>
            <file-outlined />
          </template>
          <router-link to="/logs">系统日志</router-link>
        </a-menu-item>
        <a-menu-item key="others">
          <template #icon>
            <appstore-outlined />
          </template>
          <router-link to="/others">其他页</router-link>
        </a-menu-item>
      </a-menu>
    </a-layout-sider>
    <a-layout-content style="padding: 24px; min-height: 100vh">
      <a-breadcrumb style="margin-bottom: 16px">
        <template v-for="item in breadcrumbItems" :key="item.path">
          <a-breadcrumb-item v-if="!item.children">
            <router-link :to="item.path">{{ item.title }}</router-link>
          </a-breadcrumb-item>
          <a-breadcrumb-item v-else>
            <a-dropdown>
              <a @click.prevent>
                {{ item.title }}
                <down-outlined />
              </a>
              <template #overlay>
                <a-menu @click="handleMenuClick">
                  <a-menu-item v-for="child in item.children" :key="child.path">
                    <router-link :to="child.path">{{ child.title }}</router-link>
                  </a-menu-item>
                </a-menu>
              </template>
            </a-dropdown>
          </a-breadcrumb-item>
        </template>
      </a-breadcrumb>
      <router-view></router-view>
    </a-layout-content>
  </a-layout>
</template>

<script>
import { ref, watch } from 'vue';
import { useRouter, useRoute } from 'vue-router';
import {
  HomeOutlined,
  SettingOutlined,
  FileOutlined,
  AppstoreOutlined,
  UserOutlined,
  LogoutOutlined,
  TeamOutlined,
  SafetyOutlined,
  DownOutlined
} from '@ant-design/icons-vue';

export default {
  name: "MenuBar",
  components: {
    HomeOutlined,
    SettingOutlined,
    FileOutlined,
    AppstoreOutlined,
    UserOutlined,
    LogoutOutlined,
    TeamOutlined,
    SafetyOutlined,
    DownOutlined
  },
  setup() {
    const router = useRouter();
    const route = useRoute();
    const selectedKeys = ref(['home']);
    const isDarkMode = ref(false);
    const username = ref('管理员');
    const userAvatar = ref('https://joeschmoe.io/api/v1/random');
    const openKeys = ref(['management']);  // 默认展开系统管理菜单

    const toggleTheme = (checked) => {
      // 实现暗黑模式切换逻辑
      console.log('Dark mode:', checked);
    };

    const handleLogout = () => {
      // 清除登录状态（如果有的话）
      localStorage.removeItem('token');
      // 跳转到登录页
      router.push('/login');
    };

    // 路由路径映射表
    const routeMap = {
      '/': { title: '首页' },
      '/management': { 
        title: '列表管理',
        children: [
          { path: '/management/users', title: '用户管理' },
          { path: '/management/roles', title: '角色管理' }
        ]
      },
      '/management/users': { title: '用户管理' },
      '/management/roles': { title: '角色管理' },
      '/logs': { title: '系统日志' },
      '/others': { title: '其他页' }
    };

    const handleMenuClick = ({ key }) => {
      router.push(key);
    };

    // 面包屑项
    const breadcrumbItems = ref([]);

    // 监听路由变化更新面包屑
    watch(() => route.path, (path) => {
      const paths = path.split('/').filter(Boolean);
      let currentPath = '';
      breadcrumbItems.value = [{ path: '/', title: '首页' }];
      
      paths.forEach(p => {
        currentPath += `/${p}`;
        if (routeMap[currentPath]) {
          breadcrumbItems.value.push({
            ...routeMap[currentPath],
            path: currentPath
          });
        }
      });
    }, { immediate: true });

    return {
      selectedKeys,
      isDarkMode,
      username,
      userAvatar,
      toggleTheme,
      handleLogout,
      openKeys,
      breadcrumbItems,
      handleMenuClick
    };
  }
};
</script>

<style scoped>
.header {
  background: #fff;
  padding: 0 24px;
  display: flex;
  justify-content: flex-end;
  align-items: center;
  position: fixed;
  width: calc(100% - 200px);
  right: 0;
  z-index: 1;
}

.header-right {
  display: flex;
  align-items: center;
  gap: 16px;
}

/* From Uiverse.io by JustCode14 */ 
/* Theme Switch */
/* The switch - the box around the slider */
.switch {
  font-size: 13px;
  position: relative;
  display: inline-block;
  width: 4em;
  height: 2.2em;
  border-radius: 30px;
  box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);
}

/* Hide default HTML checkbox */
.switch input {
  opacity: 0;
  width: 0;
  height: 0;
}

/* The slider */
.slider {
  position: absolute;
  cursor: pointer;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background-color: #2a2a2a;
  transition: 0.4s;
  border-radius: 30px;
  overflow: hidden;
}

.slider:before {
  position: absolute;
  content: "";
  height: 1.2em;
  width: 1.2em;
  border-radius: 20px;
  left: 0.5em;
  bottom: 0.5em;
  transition: 0.4s;
  transition-timing-function: cubic-bezier(0.81, -0.04, 0.38, 1.5);
  box-shadow: inset 8px -4px 0px 0px #fff;
}

.switch input:checked + .slider {
  background-color: #00a6ff;
}

.switch input:checked + .slider:before {
  transform: translateX(1.8em);
  box-shadow: inset 15px -4px 0px 15px #ffcf48;
}

.star {
  background-color: #fff;
  border-radius: 50%;
  position: absolute;
  width: 5px;
  transition: all 0.4s;
  height: 5px;
}

.star_1 {
  left: 2.5em;
  top: 0.5em;
}

.star_2 {
  left: 2.2em;
  top: 1.2em;
}

.star_3 {
  left: 3em;
  top: 0.9em;
}

.switch input:checked ~ .slider .star {
  opacity: 0;
}

.cloud {
  width: 3.5em;
  position: absolute;
  bottom: -1.4em;
  left: -1.1em;
  opacity: 0;
  transition: all 0.4s;
}

.switch input:checked ~ .slider .cloud {
  opacity: 1;
}


.user-info {
  display: flex;
  align-items: center;
  cursor: pointer;
}

.username {
  margin-left: 8px;
  color: rgba(0, 0, 0, 0.85);
}

.ant-layout-sider {
  position: fixed;
  height: 100vh;
  left: 0;
  top: 0;
}

.ant-layout-content {
  margin-left: 200px;
  margin-top: 64px;
}

:deep(.ant-menu-item) a {
  color: rgba(255, 255, 255, 0.65);
  text-decoration: none;
}

:deep(.ant-menu-item-selected) a {
  color: #fff;
}

:deep(.ant-breadcrumb a) {
  color: rgba(0, 0, 0, 0.45);
}
:deep(.ant-breadcrumb a:hover) {
  color: #1890ff;
}

:deep(.ant-breadcrumb-link > a) {
  display: inline-flex;
  align-items: center;
  gap: 4px;
}
</style>